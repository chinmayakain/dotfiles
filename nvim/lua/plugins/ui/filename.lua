...
return {
  {
    ...
    config = function()
      ...
      local lsp_status = require("helper.util.lsp_status")
      local workspace = require("helper.util.workspace")
          ...
          {
            {
              ...
              guibg = props.focused and extra_colors.theme_bg or extra_colors.theme_bg,
            },
            {
              icon.icon,
              " ",
              guifg = buf_focused and icon.fg or colors.deep_licorice,
      ...
      local get_file_info = function(props)
        ...
        local fname = shorten_path_styled(bufname, {
          ...
          tail_count = 2,
          head_max = 4,
          head_style = { guifg = props.focused and extra_colors.fg_nc or colors.anise_dark },
          tail_style = { guifg = props.focused and extra_colors.fg or colors.deep_licorice },
        })
        return { fname = fname }
      end
      ...
      incline.setup({
        render = function(props)
          ...
          local status = {
            { diag_disabled and " 󱒼 " or "", guifg = buf_focused and colors.deep_velvet or colors.deep_hydrengea },
            { has_error and " 󰚛 " or " ", guifg = colors.cocoa },
            lsp,
            get_pnpm_info(file_info),
            {
              file_info.fname,
              guifg = props.focused and fg or colors.deep_licorice,
              gui = modified and "bold,italic" or nil,
            },
            { modified and "  " or "", guifg = "NONE" },
            git_status(props),
            guibg = bg,
